...
class LIBEROEval:
    ...
    # ---- public API --------------------------------------------------------
    def eval_episodes(self, policy: ClientModel, save_path: Path, target_task_name: Optional[str] = None) -> float:
        ...
        for task_suite in self.task_suite_list:
            ...
            for task_id in range(len(task_suite.tasks)):
                if target_task_name and task_names[task_id] != target_task_name:
                    continue  # Skip everything else
                
                print(f"\n🚀 Found Target Task! Index: {task_id} | Name: {task_names[task_id]}")
...
def eval_libero(
    ...
    for suite_name in task_suites:
        ...
        eval_rewards = evaluator.eval_episodes(agent, save_path=save_path, target_task_name=target_task_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task_name", type=str, default="pick_up_the_black_bowl_between_the_plate_and_the_ramekin_and_place_it_on_the_plate_demo", 
                        help="Specify a single task name to run within the suite")
    ...
    try:
        eval_results = eval_libero(
            ...
            target_task_name=args.task_name,
        )
